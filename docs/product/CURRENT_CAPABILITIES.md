...
| G8 READ runtime pack implementation | IMPLEMENTED | merged implementation + owner-authorized alternative external Linux READ acceptance for `main@2f9ab7fdfe8793a9b2c977bc620c0f10921f4e3f` | not installed by default; official GitHub Actions parity pending due account-level Actions policy |
...
| Real canonical HTTP READ E2E + restart resume | VERIFIED | owner-authorized alternative external Linux evidence: authenticated HTTP→Runner→independent `VerificationResult/v1` plus ACTIVE interruption/resume and no duplicate authority/effect after restart | official GitHub Actions parity pending; no WRITE, release, deployment or production effects authorized |
